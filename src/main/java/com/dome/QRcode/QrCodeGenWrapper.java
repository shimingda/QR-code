...
public class QrCodeGenWrapper {
     ...
     */
    public static BitMatrix encode(String data, String charset, Map<EncodeHintType, ?> hint, int width,
                                    int height) throws WriterException {
        ErrorCorrectionLevel errorCorrectionLevel = ErrorCorrectionLevel.L;
        int quietZone = 1;
        if (hint != null) {
            if (hint.containsKey(EncodeHintType.ERROR_CORRECTION)) {
                errorCorrectionLevel = ErrorCorrectionLevel.valueOf(hint.get(EncodeHintType.ERROR_CORRECTION).toString());
            }
            if (hint.containsKey(EncodeHintType.MARGIN)) {
                quietZone = Integer.parseInt(hint.get(EncodeHintType.MARGIN).toString());
            }
        ...
        QRCode code = Encoder.encode(data, errorCorrectionLevel, hint);
